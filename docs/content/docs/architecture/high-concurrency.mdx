...
- **短链系统**：发号器生成 ID → Base62 编码 → 短码映射存 MySQL → Redis 热点缓存 + 布隆过滤器拦截不存在短码
- **分布式 ID**：Snowflake（时间戳+机器ID+序列号，有序高性能，依赖时钟）→ 号段模式（DB 批量分配，不依赖时钟）→ UUID v7（本地生成，时间排序）
- **计数系统**：Redis INCR 基础计数 → 异步合并写（本地累加满 100 批量 INCRBY）→ HyperLogLog（UV 估算，12KB 统计 2^64）→ Sorted Set（排行榜）
- **四种限流算法**：固定窗口（边界翻倍）→ 滑动窗口（平滑精确）→ 漏桶（恒定流出）→ 令牌桶（允许突发，最常用）
- **多层限流**：Nginx limit_req（漏桶，IP 级）→ API Gateway（令牌桶，接口级）→ 应用 Sentinel（方法级+热点参数）
- **限流 vs 熔断 vs 降级**：限流保护自身（拒多余请求），熔断防止级联（快速失败），降级有损提供（兜底结果）
...
1. **Snowflake 的时钟回拨问题有哪些解决方案？**
   <br/>→ 短时间回拨等待时钟追上（自旋/休眠）；配置最大容忍回拨时间（如 5ms）；美团 Leaf 方案：号段模式不依赖时钟 + Snowflake 模式用 ZK 持久化最大时间戳；备用 workerId 切换。

2. **固定窗口限流的"边界翻倍"问题是什么？滑动窗口如何解决？**
   <br/>→ 固定窗口在窗口边界处没有区分（如第 59 秒和前 1 秒都在"同一分钟"窗口内），可能导致窗口切换瞬间流量翻倍。滑动窗口将窗口细分为多格，随时间滑动统计，杜绝边界问题。

3. **为什么推荐令牌桶而非漏桶？什么场景适合漏桶？**
   <br/>→ 令牌桶允许一定突发流量（可积累令牌），更贴合实际业务流量模式。漏桶强制恒定速率输出，适合需要严格流量整形、保护下游弱依赖的场景。

4. **短链系统中布隆过滤器的作用是什么？误判会有什么影响？**
   <br/>→ 拦截不存在的短码请求，避免每次无效请求都穿透到 DB。误判会导致"不存在的短码被判定为可能存在"，走 Redis→DB 的正常查询流程，不会返回错误结果（多了一次 DB 查询，影响可控）。
