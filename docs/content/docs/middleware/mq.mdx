...
Producer 超时，无法区分：
  ...
  情况 2：消息到了，ACK 丢了 → 重试会重复！

Kafka 幂等 Producer 解决方案：
  Broker 给每个 Producer 分配 PID，每条消息带 Sequence Number
  Broker 检测到重复 seq → 丢弃，返回 ACK
  → Producer 放心重试，不会重复

兜底：业务消息体带唯一 msgId，Consumer 侧 Redis/DB 去重
```
