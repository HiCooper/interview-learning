...
- **Dubbo 3.x**：Triple 协议（gRPC，支持 Streaming）→ 应用级服务发现（降低注册中心压力）→ 云原生 Kubernetes
- **RPC 全链路**：Proxy → Directory → Router → LoadBalance → Filter Chain → Protocol → Netty，序列化+网络传输
- **集群容错**：Failover（默认，失败重试）→ Failfast → Failsafe → Failback → Forking → Broadcast
- **负载均衡**：Random（权重随机）→ RoundRobin → LeastActive → ConsistentHash（同参数同提供者）
- **Dubbo SPI vs JDK SPI**：按需加载（key-value 指定）→ Wrapper 增强（AOP）→ 自适应扩展（@Adaptive）→ 自动激活（@Activate）
## 架构核心
...
1. **Dubbo RPC 调用全链路经过哪些核心组件？**
   <br/>→ Proxy（动态代理）→ Cluster Invoker → Directory（缓存提供者列表）→ Router（路由过滤）→ LoadBalance（选择实例）→ Filter Chain（过滤器链）→ Protocol → Exchange → Transport → Netty（网络传输）。

2. **Dubbo SPI 相比 JDK SPI 有哪些增强？**
   <br/>→ JDK SPI 一次性全加载；Dubbo SPI 按需加载（key-value 指定）、支持 AOP（Wrapper 机制）、支持 IoC（依赖注入）、自适应扩展（@Adaptive 根据 URL 参数动态选择实现）、自动激活（@Activate + group + order）。

3. **Dubbo 的 Failover 和 Failfast 容错策略各适合什么场景？**
   <br/>→ Failover（默认）：失败自动切换重试，适合幂等操作（查询类）。Failfast：立即失败报错，适合非幂等写操作（避免重复写入）。实际中写操作通常用 Failfast + 业务层幂等保证。
